function TOPress_press_chamber(nelx,nely,volfrac,penal,rmin,etaf,betaf,ls,maxit)
...
%% ____PART 2.______________________FINITE ELEMENT ANALYSIS PREPARATION
...
elNrs = reshape(1:nel,nely,nelx); % reshaping the element
s1 = elNrs(3*nely/8:17*nely/40, 2*nelx/3:nelx); % solid elements
s2 =  elNrs(23*nely/40:5*nely/8, 2*nelx/3:nelx); % solid elements
v1 = elNrs(17*nely/40:end,7*nelx/15:8*nelx/15); % void elements
v2 = elNrs(17*nely/40:23*nely/40,8*nelx/15:nelx); %% solid elements
[NDS, NDV] = deal( [s1(:);s2(:)], [v1(:); v2(:)] );  % total solid and void elements    
act = setdiff( (1 : nel)', union( NDS, NDV ) ); % active set of elements
s1fix = elNrs(3*nely/8:17*nely/40, nelx); %fixed part of the solid
s2fix = elNrs(23*nely/40:25*nely/40, nelx); % fixed part of the solid
fixx = unique(Pdofs([s1fix(:); s2fix(:)],:)); % total fixed
%% ____PART 3.______PRESSURE & STRUCTURE B.C's, LOADs; NON-DESIGN DOMAIN
...
while(loop<maxit && change>0.01)
    ...
    objsT2 = dC1k + dC1h; objsens = (objsT1 + ls*objsT2); % final sensitivities
...
node = [ (1:nno)' reshape(repmat(0:nelx,nely+1,1),nno,1) repmat(0:-1:-nely,1, nelx+1)'];
[yy,xx,nod_dV] = deal(node(Lnode,3), node(Bnode,2),zeros(nno,1));
