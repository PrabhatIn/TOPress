function TOPress_press_chamber_projected(nelx,nely,volfrac,penal,rmin,etaf,betaf,betamax,lst,maxit)
...
s1 = elNrs(3*nely/8:17*nely/40, 2*nelx/3:nelx);
s2 =  elNrs(23*nely/40:5*nely/8, 2*nelx/3:nelx);
v1 = elNrs(17*nely/40:end,7*nelx/15:8*nelx/15);
v2 = elNrs(17*nely/40:23*nely/40,8*nelx/15:nelx);
[NDS, NDV] = deal( [s1(:);s2(:)], [v1(:); v2(:)] );      
...
[nMMA,mMMA,xTilde,xMMA,mvLt] = deal(length(act),1,x,x(act),0.1);
...
while(loop<maxit && change>0.01)
    ...
    objsens = imfilter(reshape(objsens*normf, nely, nelx)./dHs,h);
    ...
    colormap(gray); imagesc(1-reshape(xphys, nely, nelx));clim([0 1]);axis equal off;drawnow;
end
Mnd = 4*sum(xphys.*(1-xphys))/(nelx*nely); % determining the gray scale 
